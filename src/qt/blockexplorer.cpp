...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PYMT, nValue);
...
{
    std::string genesisblockhash = "00000cc42b8de9b94ff49d7738865e306c50d8f7a0f3af09d23a3e3cc6c253c4";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (PYMT.conf).");
            QMessageBox::warning(this, "PYMT Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
